...
/**
 * 获取 /2025作品/单人特写 文件夹下的文件列表
 */
export const fetch2025WorksFilebyFolder = async ( folderName:string ) => {
  const token = getAccessToken();
  if (!token) throw new Error("No access token, please authorize first");
  ...
  try {
    ...
    /** Step 2：找到 server_filename === "string" 的文件夹 */
    const targetFolder = rootData.list.find(
      (item: any) => item.isdir === 1 && item.server_filename === folderName
    );

    if (!targetFolder) {
      throw new Error(`未找到 ${folderName} 文件夹`);
    }
